...
import { PlayCircle, Target, Clock, BookOpen, AlertCircle, ChevronRight } from 'lucide-react';
import ExamSidebar from '../components/ExamSidebar';
import { motion } from 'framer-motion';

const CoreTest = ({ setCurrentView }) => {
  const startModule = (moduleId) => {
    localStorage.setItem('selectedSimulatorModule', moduleId);
    setCurrentView('Simulator');
  };

  const startFullTest = () => {
    localStorage.removeItem('selectedSimulatorModule');
    setCurrentView('Simulator');
  };
      ...
      <main style={{ flex: 1, padding: '48px', overflowY: 'auto' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          ...
          <header style={{ textAlign: 'center', marginBottom: '56px', marginTop: '32px' }}>
            <h1 style={{ fontSize: '3rem', fontWeight: 800, color: 'var(--primary)', marginBottom: '16px', letterSpacing: '-1px' }}>TestAS Mock Test</h1>
            <p style={{ fontSize: '1.25rem', color: 'var(--text-muted)', maxWidth: '600px', margin: '0 auto', lineHeight: 1.6 }}>
              The TestAS measures cognitive abilities necessary for success in German university programs.
            </p>
          </header>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '20px', marginBottom: '48px' }}>
            {[
              { label: 'Number of Sections', value: '4', icon: <BookOpen size={20} /> },
          ...
          <div className="premium-card" style={{ padding: '32px', marginBottom: '48px', textAlign: 'center', background: 'rgba(255, 212, 130, 0.1)', border: '1px solid var(--border)' }}>
            <div style={{ fontSize: '1.1rem', fontWeight: 600, color: 'var(--primary)' }}>Difficulty: Adaptive Cognitive Reasoning</div>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '80px' }}>
            ...
            >
              <PlayCircle size={24} /> Start Full TestAS Mock
            </button>
          </div>
