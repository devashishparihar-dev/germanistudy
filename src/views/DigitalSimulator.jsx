...
import { Flag, CheckCircle, ChevronRight, PanelRightClose, PanelRightOpen, Monitor, AlertCircle } from 'lucide-react';
...
const DigitalSimulator = ({ setCurrentView }) => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        setLoading(true);
        const selectedModule = localStorage.getItem('selectedDigitalModule');
  ...
  const handleSectionComplete = () => {
    const isSingleModule = !!localStorage.getItem('selectedDigitalModule');
  ...
  useEffect(() => {
    if (isExamFinished) {
      ...
      const isSingleModule = !!localStorage.getItem('selectedDigitalModule');
      const examName = isSingleModule ? `Digital TestAS Core - ${SECTION_CONFIG[currentSectionIndex].title}` : 'Digital TestAS Core - Full Mock';
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'var(--bg-main)' }}>
        <header style={{ height: '80px', background: 'rgba(255,255,255,0.02)', backdropFilter: 'blur(10px)', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 32px', position: 'sticky', top: 0, zIndex: 10 }}>
          <div style={{ fontWeight: 700, fontSize: '1.25rem', color: 'var(--primary)' }}>Germanistudy - Digital Test Results</div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '48px 10%' }}>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="premium-card" style={{ padding: '48px', textAlign: 'center', marginBottom: '48px' }}>
            <CheckCircle size={64} style={{ color: 'var(--success)', margin: '0 auto 24px' }} />
            <h2 style={{ fontSize: '2.5rem', marginBottom: '16px', color: 'var(--text)' }}>Digital Exam Completed</h2>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'var(--background)' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="premium-card" style={{ background: 'var(--surface)', padding: '32px', maxWidth: '500px', width: '100%', textAlign: 'center' }}>
            ...
            <p style={{ color: 'var(--text-muted)', marginBottom: '32px', fontSize: '1.1rem' }}>
              Are you sure you want to finish this section? You will not be able to return to any questions. {unansweredCount > 0 ? `You have ${unansweredCount} unanswered question(s).` : 'All questions are answered.'}
            </p>
      ...
      <header style={{ 
        ...
        <div style={{ width: '30%', display: 'flex', alignItems: 'center', gap: '24px' }}>
          <div style={{ fontWeight: 800, fontSize: '1.25rem', color: 'var(--primary)', letterSpacing: '-0.5px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Monitor size={20} />
            GermaniStudy Digital
          </div>
      ...
      <div style={{ background: 'rgba(239, 68, 68, 0.05)', borderBottom: '1px solid var(--border)', padding: '8px', textAlign: 'center', color: 'var(--error)', fontSize: '0.85rem', fontWeight: 600, display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px' }}>
        <AlertCircle size={16} /> Strict Mode: No backwards navigation allowed.
      </div>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden', position: 'relative' }}>
        <main style={{ flex: 1, display: 'flex', flexDirection: 'column', position: 'relative', overflowY: 'auto' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '24px 10%', background: 'var(--background)', marginTop: 'auto' }}>
            ...
            <button className="btn-primary" onClick={handleNextClick} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '12px 32px' }}>
              {currentQIndex === currentSectionQuestions.length - 1 ? 'Review Section' : 'Next (No Return)'} <ChevronRight size={20} />
            </button>
          </div>
        </main>
              ...
              <div style={{ flex: 1, padding: '24px', overflowY: 'auto' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '12px' }}>
                  {currentSectionQuestions.map((q, idx) => {
                    ...
                    if (isPast && !isAnswered && !isFlagged) {
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          border: border, fontWeight: 600, fontSize: '0.9rem',
                          display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s',
                          opacity: isPast ? 0.6 : 1
                        }}
                        title={isPast ? "You cannot return to previous questions." : ""}
                      >
                        {idx + 1}
                      </div>
                    );
